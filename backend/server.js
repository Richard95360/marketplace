...
const uri = 
"mongodb+srv://richardtest:test1234@cluster-marketplace.8k9je.mongodb.net/marketplace?retryWrites=true&w=majority";
...
client.connect(err => {
  ...
  !err && console.log(`SucessFully connected to database marketplace`);
...
//Post
 ...
 routes.post("/products/add",jsonParser, function (req,res) {
   products
   .insertOne(req.body)
   .then(() =>  res.status(200).send('sucessfully insert new document'))
   .catch((err) => {
    console.log(err);
       res.send(err)
    });
